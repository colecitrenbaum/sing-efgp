"""Subprocess-isolated bench mirroring the K-sweep config from
``bench_gpdrift_K_sweep.py``: K=10, T=1000, N_EM=50, N_ESTEP=10. Runs
all three ``qx_moments_method`` modes (gmix_live / gmix_batched /
linearised_shim) in fresh subprocesses for clean wall numbers.

This is the apples-to-apples test for the earlier observation that the
K-sweep timing (2.23s/iter at T=1000) is much faster than today's
gmix_live timing (6.77s/iter at T=500) — strong evidence yesterday's
K-sweep was NOT actually using gmix_live (consistent with the file-
timestamp analysis: efgp_gmix_qx_moments.py was last modified after
the K-sweep run).
"""
...
def worker(args):
    ...
    # Mirror the K-sweep config exactly
    D = 2; SIGMA = 0.4
    LS_TRUE, VAR_TRUE = 0.6, 1.0
    LS_INIT, VAR_INIT = 0.3, 1.0           # K-sweep used 0.3 init
    N_OBS = 6
    ALPHA_RESTORE = 0.05                    # K-sweep weak restoring
    ...
    # GP-drift draw (extent matches K-sweep's 16-pt template at [-3, 3])
    X_template = jnp.linspace(-3.0, 3.0, 16)[:, None] * jnp.ones((1, D))
    grid = jp.spectral_grid_se(LS_TRUE, VAR_TRUE, X_template, eps=1e-3)
    M = grid.M
    ...
    def drift(x, t):
        # GP drift + weak linear restoring (K-sweep convention)
        return gp_drift(x, t) - ALPHA_RESTORE * x
    ...
    # Single fit only — avoids the cold/warm subtlety. Pure wall-to-answer.
    t0 = time.time()
    mp, _, _, _, hist = _fit()
    wall = time.time() - t0
    ...
    out = dict(
        ...
        wall=wall, per_iter=wall / args.n_em,
        ls=float(hist.lengthscale[-1]),
        var=float(hist.variance[-1]),
        latent_rmse=float(hist.latent_rmse[-1]),
    )
    print(json.dumps(out))


def orchestrator():
    # Yesterday's K-sweep config
    K, T, T_MAX = 10, 1000, 15.0
    N_EM, N_ESTEP = 50, 10
    modes = ['linearised_shim', 'gmix_batched']  # cheapest first

    py = sys.executable
    script = str(Path(__file__).resolve())
    rows = []

    print(f"=== K-sweep config (K={K}, T={T}, N_EM={N_EM}, N_ESTEP={N_ESTEP}) ===")
    print(f"  Yesterday's K-sweep K=10 wall: 111.5s (per-iter 2.23s)")
    print(f"  Hypothesis: that was linearised_shim, not gmix_live.\n")
    ...
    for mode in modes:
        ...
        rows.append(data)
        print(f"  {mode:>16s}: wall={data['wall']:6.1f}s  "
              f"per-iter={data['per_iter']:.3f}s  "
