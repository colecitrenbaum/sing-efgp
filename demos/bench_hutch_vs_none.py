"""Subprocess-isolated bench: 'none' vs 'hutch' vs 'hutch_hetS' for the
``restore_qf_variance`` knob, on the production K=10/T=500 problem.

All three modes use ``qx_moments_method='gmix_batched'`` (the new default).
Each cell runs in a fresh subprocess so JIT compile + library init are
paid cleanly per mode (no cross-mode cache sharing).
"""
...
def worker(args):
    ...
    def _fit():
        ...
        return em.fit_efgp_sing_jax(
            ...
            qx_moments_method='gmix_batched',           # new default
            restore_qf_variance=args.mode,
        )

    # Cold + warm to separate JIT compile from steady-state compute.
    t0 = time.time()
    _, _, _, _, hist1 = _fit()
    cold = time.time() - t0
    t0 = time.time()
    ...
    out = dict(
        ...
        cold_wall=cold, warm_wall=warm, jit_cost=cold - warm,
        per_iter_warm=warm / args.n_em,
...
def orchestrator():
    ...
    print(f"=== restore_qf_variance comparison (K={K}, T={T}, "
          f"qx_moments_method='gmix_batched') ===")
    print(f"  Each mode = fresh subprocess. cold = first fit, "
          f"warm = second fit (JIT cached).\n")
    ...
    print("\n--- warm wall ratios (relative to 'none') ---")
    none = next((r['warm_wall'] for r in rows if r['mode'] == 'none'), None)
    if none:
        for mode in modes:
            cell = next((r for r in rows if r['mode'] == mode), None)
            if cell:
                print(f"  {mode}/none = {cell['warm_wall']/none:.2f}x")
