...
\noindent
\textbf{Placement.} This whole note lives inside departure \textbf{D1} of
\texttt{efgp\_estep.tex} \S2 (``$q(f)$ is a Fourier-feature Gaussian
updated in closed form''): the M-step is the extra block that generic SING
has no analogue for. It runs once per outer EM iteration, but only after
\code{kernel\_warmup\_iters} (default 8) so that $q(f)$ is not still
trivial. \S\ref{sec:leftout}(B1) and (B2) below are the same objects as
the E-step doc's departures \textbf{D3} and \textbf{D2} respectively.
...
$\varepsilon_i\sim\mathcal N(0,\Delta_i\Sigma)$, and $\delta_i:=x_{i+1}-x_i$.
(As in the E-step doc, the \emph{code} is the isotropic case
$\Sigma=\sigma^2I$: \code{sigma\_drift\_sq} is a scalar, so every
$\sigma_r^{-2}$ below is one $\sigma^{-2}$.) The ELBO is
\begin{equation}
\begin{aligned}
...
$F_x,\ D_\theta$ & Fourier features $F_x[k]{=}e^{2\pi i x^\top\xi_k}$; \ $D_\theta{=}\diag(\sqrt{S(\xi_k;\theta)\,h^D})$ (real, diagonal; \code{grid.ws}). \\
...
\qquad(\texttt{efgp\_jax\_drift.py:907}).
\label{eq:specdens}
\end{equation}
\code{\_ws\_real\_se} evaluates $\sqrt{S(\xi_k)h^D}$ in log-space, so the
square-root's gradient stays finite at the high-frequency modes where
$S(\xi_k)$ underflows to zero.
...
(\texttt{efgp\_jax\_drift.py:925--1029}), using
$\langle\mu,A\mu\rangle=\langle\mu,h\rangle$ to collapse the quadratic.
This is the E-step doc's eq.~(Mstep-collapsed).

\paragraph{How it is actually optimised (v0: dense Cholesky).}
$\theta=(\log\ell,\log\sigma_f^2)$ is moved by \code{n\_mstep\_iters}
Adam steps on \eqref{eq:Mstep}. Each step materialises the \emph{dense}
$A_\theta = I + \diag(\text{ws})\,T\,\diag(\text{ws})$ ($O(M^2)$, by
direct indexing into the cached BTTB convolution vector) and takes ONE
Cholesky $A_\theta=LL^*$, which supplies both
$\log|A_\theta|=2\sum_k\log L_{kk}$ and $\mu_\theta=A_\theta^{-1}h_\theta$
by two triangular solves; \code{jax.grad} then differentiates the whole
thing, handling the implicit $\mu_\theta$ by the envelope theorem. Cost
$O(M^3/3 + \Dt M^2)$ per Adam step, practical for $M\lesssim 2000$--$4000$.
\emph{The Hutchinson trace path is gone}: \code{n\_hutchinson},
\code{include\_trace}, \code{cg\_tol\_trace}, \code{max\_cg\_iter\_trace}
and \code{key} are still accepted for signature compatibility but
\textbf{ignored}, so \code{n\_hutchinson\_mstep} in a driver script is
inert. (Repo history holds the old estimator; $P\!\ge\!64$ probes were
needed to match exact slogdet on this problem.) Two more v0 details worth
knowing: the Adam parameters are cast to \code{float32} inside
\code{m\_step\_kernel\_jax} even under \code{jax\_enable\_x64}, and
\code{efgp\_em.py:1074} clamps the accepted step to
$\ell\in(0.05,50)$, $\sigma_f^2\in(0.01,100)$ as a collapse guard.
...
(\texttt{efgp\_jax\_drift.py:994,1003}).

\emph{Two implementation caveats on ``frozen''.} (a) $z_r$ is not
accumulated during the RHS build; the driver recovers it from the
converged E-step solution as
$z_r = (A_{\theta_0}\mu_r)/\text{ws}(\theta_0)$
(\texttt{efgp\_em.py:1051--1060}) --- the same object by
$h_{\theta,r}=D_\theta z_r$, but obtained by re-applying $A$ at the
\emph{incoming} hypers $\theta_0$. (b) The spectral grid
$\{\xi_k\}$ and spacing $h$ are themselves tailored to $\ell$
(\code{eps\_grid}) and rebuilt \emph{after} each accepted M-step. So
$(T,z_r)$ are $\theta$-free \emph{within} one M-step --- which is all
\eqref{eq:Mstep} needs --- but not across outer iterations.

So \eqref{eq:Mstep} is precisely a GP
...
(= E-step doc departure \textbf{D3}: \code{ff} returns
$\|\Eqx[\bar f]\|^2$, not $\|\bar f\|^2{+}V$).
...
(= E-step doc departure \textbf{D2}; concretely, the
$\partial/\partial S_i$ column of that doc's six-slot interface table).
...
\;\overset{\text{cov-drop}}{\approx}\;\Eqx[J_{\bar f}]^\top\Sigma^{-1}\Eqx[J_{\bar f}],\\[2pt]
&\text{dropping }\;
\dropped{\textstyle\sum_r\sigma_r^{-2}\bar f_r H_{\bar f_r}}
\;\text{ and }\;\dropped{\Cov(J)}.
\end{aligned}
\end{equation}
...
\textbf{free}: $\tr A_\theta^{-1}{=}\|L^{-1}\|_F^2$ reuses the Cholesky $A_\theta{=}LL^*$ that \code{m\_step\_kernel\_jax} already forms; and it is already implicit in $\log|A_\theta|$. \\
